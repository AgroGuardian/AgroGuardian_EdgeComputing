...
[cite_start]Este repositório contém o desenvolvimento da camada de **Edge Computing & Computer Systems** do projeto **AgroGuardian**, uma plataforma focada em transformar o manejo agrícola reativo em gestão preditiva[cite: 23]. O protótipo foi inteiramente desenvolvido e validado de forma funcional utilizando o simulador **Tinkercad**.

---

## 📋 Descrição do Projeto
[cite_start]O **AgroGuardian Edge** é uma estação meteorológica e de monitoramento de solo automatizada projetada para operar diretamente nas extremidades (campo) da propriedade agrícola[cite: 20]. [cite_start]Enquanto a plataforma central processa dados macro (como imagens orbitais e índices de vegetação NDVI) [cite: 20, 186][cite_start], este dispositivo atua na coleta microclimática em tempo real[cite: 79]. 
...
## 🎯 Objetivo da Solução
* [cite_start]**Antecipação de Riscos:** Identificar preventivamente condições favoráveis a geadas, ondas de calor extremo e estresse hídrico (seca) antes que causem perdas produtivas[cite: 13, 86].
* [cite_start]**Manejo Inteligente:** Fornecer dados locais precisos para otimizar os ciclos de irrigação, alinhando-se aos Objetivos de Desenvolvimento Sustentável da ONU (ODS 2 - Agricultura Sustentável e ODS 13 - Ação Contra a Mudança Global do Clima)[cite: 22, 74].
...
| **Display LCD 16x2** | [cite_start]Interface visual local para exibição das leituras em tempo real[cite: 79]. | Pinos Digitais 12, 11, 5, 4, 3, 2 |
| **Sensor de Temperatura (TMP36)** | [cite_start]Monitora as oscilações térmicas do ar para prever riscos de geada[cite: 86]. | Pino Analógico A1 |
| **Sensor de Umidade do Solo** | [cite_start]Mede o estresse hídrico diretamente na raiz da cultura monitorada[cite: 86]. | Pino Analógico A0 |
...
| **LED Vermelho** | [cite_start]Indicador de risco crítico iminente (Seca ou Geada)[cite: 86]. | Pino Digital 10 |
...
[cite_start]O algoritmo processa as informações coletadas na ponta e as categoriza em quatro estados lógicos baseado no cruzamento de dados exigidos pelo Dashboard do AgroGuardian[cite: 86, 94]:

1. [cite_start]**Risco Crítico de Seca (Estresse Hídrico):** Se a umidade do solo cair abaixo de **30%**, o sistema exibe `ALERTA: Solo Seco` no LCD, acende o **LED Vermelho** e dispara o **Buzzer** intermitente[cite: 86].
2. [cite_start]**Risco Crítico de Geada (Estresse Térmico):** Se a temperatura ambiente detectada pelo TMP36 for inferior a **5°C**, o sistema exibe `ALERTA: Geada!` no LCD, acende o **LED Vermelho** e aciona o **Buzzer**[cite: 86].
...
> [cite_start]🌐 **Integração com a Nuvem:** Além do controle local, o Arduino envia os dados formatados continuamente via protocolo Serial (`TEMP_C` e `UMID_SOLO`), simulando a alimentação em tempo real do banco de dados da plataforma AgroGuardian[cite: 20, 79].
...
5. [cite_start]Abra o painel **"Monitor Serial"** na parte inferior da tela de código para visualizar as strings de dados sendo transmitidas em tempo real[cite: 79].
...
---
[cite_start]*Global Solution 2026 - FIAP - 1ESPG* [cite: 1, 2, 6, 8]
